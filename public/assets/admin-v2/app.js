...
function renderShell() {
  ...
  app.innerHTML = '<div class="admin-shell"><aside><div class="brand"><span class="mark">X</span><span>' + esc(window.xboardAdmin?.title || 'Xboard') + '<small>自主后台 · 第一阶段</small></span></div>' +
    '<nav><button class="nav-link active" data-page="users">用户管理</button><button class="nav-link" data-page="plans">套餐管理</button><button class="nav-link" data-page="nodes">节点管理</button><button class="nav-link" data-page="orders">订单管理</button></nav><p class="sidebar-note">只显示已经实现且可写入的模块，不用空白页面充数。</p></aside>' +
  ...
  const renderPage = page => {
    ...
    app.querySelector('.admin-topbar strong').textContent = page === 'plans' ? '套餐管理' : (page === 'nodes' ? '节点管理' : (page === 'orders' ? '订单管理' : '用户管理'));
    (page === 'plans' ? renderPlans : (page === 'nodes' ? renderNodes : (page === 'orders' ? renderOrders : renderUsers)))(root);
  };
